...
local function isAllowedOption(amount, optionKey, fallback)
    for _, option in ipairs((Config.AdDuration or {})[optionKey] or fallback) do
...
RegisterNetEvent('ec_lifeinvader:server:teamGrantAdDuration', function(requestId, targetIdentifier, amount)
    ...
    if not isAllowedOption(amount, 'teamGrantOptions', { 1, 7 }) then
...
RegisterNetEvent('ec_lifeinvader:server:teamRemoveAdDuration', function(requestId, targetIdentifier, amount)
    local src = source
    ...
    if not isAllowedOption(amount, 'teamRemoveOptions', { 1, 7 }) then
        LiBridgeServerTeam.Respond(src, requestId, { ok = false, error = 'invalid_grant_option' })
        return
    end

    LiBridgeServerAdDuration.RemoveBonusDays(targetIdentifier, amount, function(ok, reason, info)
        if not ok then
            LiBridgeServerTeam.Respond(src, requestId, { ok = false, error = reason or 'db_failed' })
            return
        end
