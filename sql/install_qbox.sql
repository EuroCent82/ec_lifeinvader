-- =============================================================================
-- ec_lifeinvader — QBox · Datenbank-Schema (6 Tabellen)
-- Resource: ec_lifeinvader
-- Framework: QBox (qbx_core) · Config.Adapters.framework = 'qbox'
-- Import:   mysql -u user -p datenbank < install_qbox.sql
--
-- Spalte `identifier` = PlayerData.citizenid (z.B. ABC12345)
-- Bridge: LiBridge.Server.GetIdentifier(source) → citizenid
-- Config: Config.Adapters.framework = 'qbox'
-- =============================================================================
...
CREATE TABLE IF NOT EXISTS `lifeinvader_feeds` (
    `id` INT UNSIGNED NOT NULL AUTO_INCREMENT,
    `identifier` VARCHAR(128) NOT NULL COMMENT 'QBox citizenid (Besitzer)',
...
CREATE TABLE IF NOT EXISTS `lifeinvader_voucher_redemptions` (
    ...
    `identifier` VARCHAR(128) NOT NULL COMMENT 'QBox citizenid',
...
CREATE TABLE IF NOT EXISTS `lifeinvader_refunds` (
    ...
    `identifier` VARCHAR(128) NOT NULL COMMENT 'QBox citizenid (Empfänger)',
    `amount` INT NOT NULL,
    `reason` VARCHAR(255) NULL,
    `issued_by` VARCHAR(128) NOT NULL COMMENT 'QBox citizenid (Team)',
    `created_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY (`id`),
